...
export async function buildReportData(
  ...
  const overview = await queryOverviewAnalytics(filters);
  ...
  const entities: EntityReportSection[] = await Promise.all(
    entitiesToInclude.map(async (entity) => {
      const entityFilters: NewsFilters = { ...filters, entity };
      const analytics = await queryEntityAnalytics(entityFilters, entity);

      const selectedRecords =
        selectedIds.length > 0
          ? await queryRecordsByIds(entityFilters, selectedIds)
          : [];
